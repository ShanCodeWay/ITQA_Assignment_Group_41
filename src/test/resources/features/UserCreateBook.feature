Feature: User Creates a Book - 205010A
  ...
  Scenario: Verify the API returns 401 for unauthorized access
    ...
    Then User should receive a failed response with status code 401 and error message "Unauthorized access"

  Scenario: User tries to create a book with the wrong password
    Given User has the base URI set to "http://localhost:7081"
    And User authenticate as "user" with password "wrongpassword"
    When User tries to create a book with title "Test Book" and author "Test Author"
    Then User should receive a failed response with status code 401 and error message "Invalid credentials"

  Scenario: User without access tries to create a book (assuming user do not have access for POST)
    Given User has the base URI set to "http://localhost:7081"
    And User authenticate as "user" with password "password"
    When User try to create a book with title "Test Book 05" and author "Author 05"
    Then User should receive a failed response with status code 403 and error message "Access denied"